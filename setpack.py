...
class Agent():
    ...
    def mupdate(self, J, s1, s2):
        if s1 in self.B:
            self.B.remove(s1)
        if s2 in self.B:
            self.B.remove(s2)
            
        s = set([obj for obj in J.i])
        s = s.union(self.i)
        for i in S:
            if s.issubset(i):
                self.B.add(J)
    ...
    def dupdate(self, M, o):
        ...
        if not a:
            return
        self.B.remove(a)
        s = set([obj for obj in M.i])
        s = s.union(self.i)
        for i in S:
            if M not in self.A or self not in M.A and s.issubset(i):
                self.B.add(M)
                return
    ...
    def start(self):
        ...
        self.B = set()
        for k in mu:
            if k == self or k in self.children:
                continue
        ...
        if self.i in S and self not in nu:
            self.B.add(self)
    ...
    def apply(self,J):
        ...
        if J.prefers(self, J.sibling):
            ...
            for k in sigma:
                if k not in multiset.children:
                    k.mupdate(multiset, self, J)
